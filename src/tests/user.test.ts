...
describe("POST /user/login", ()=>{
   describe("given email and password", ()=>{
      test("should respond with a 401 code to wrong formated email", async () => {
...
describe("POST /user/registerAdmin", ()=>{
   describe("given email and password", ()=>{
      test("should respond with a 401 code to wrong formated email", async () => {
         const response = await request(app).post("/user/registerAdmin").send({
           email: "asd123",
           password: "w/e"
         })
         expect(response.statusCode).toBe(401)
